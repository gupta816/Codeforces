...
public void close() throws IOException 
    ...
    public static void main(String args[])throws IOException
    {
    	...
    	while(tc--!=0)
    	{
    	    ...
    	    long[] c=new long[n];
    	    long[] o=new long[n];
    	    long cini=9223372036854775807L;
    	    long oini=9223372036854775807L;
    	    
    	    for(int i=0;i<n;i++)
    	    {
    	        c[i]=inputLong();
    	        if(c[i]<=cini) cini=c[i];
    	    }
       	    for(int i=0;i<n;i++)
    	    {
    	        o[i]=inputLong();
    	        if(o[i]<=oini) oini=o[i];
    	    }
    	    ...
    	    for(int i=0;i<n;i++)
    	    {
    	        ...
    	        if(o[i]!=oini && c[i]!=cini)
    	        {
    	            ans+=Math.max(o[i]-oini,c[i]-cini);
    	        }
    	        else if(o[i]!=oini)
    	        {
    	            ans+=o[i]-oini;
